...
window.addEventListener("DOMContentLoaded", () => {
  ...
  uploadCards.forEach((card) => {
    ...
    const fallbackDefaultMessage =
      fallbackMessage?.dataset.defaultMessage ?? "";
    ...
    input.addEventListener("change", () => {
      ...
      console.log("[PublishModule] File preview check", {
        inputName: input.name,
        isPdf,
      });
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const assignmentUpload =
      document.getElementById("assignment-upload").files[0];
    ...
    if (!assignmentUpload)
      return alert("Please select an assignment to upload");
    ...
    try {
      ...
      console.log(
        "[PublishModule] Sending fetch request to /api/upload_moderation"
      );
      const options = {
        method: "POST",
      ...
      if (res.status !== 200 || data.error) {
        ...
        console.log("[PublishModule] Navigating after successful publish", {
          moduleId,
        });
        if (moduleId) {
      ...
      alert(
        "Network or server error occurred while publishing the module. Check console for details."
      );
    }
  });
});
