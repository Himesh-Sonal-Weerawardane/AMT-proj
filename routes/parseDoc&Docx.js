...
function getFirstLine(xmlJsonData) {
  try {
    ...
    const firstParagraph = Array.isArray(paragraphs)
      ? paragraphs[0]
      : paragraphs;
...
export default function (props) {
  return new Promise((resolve, reject) => {
    ...
    if (!existsSync(props.file)) {
      reject(
        new Error(
          `Input file "${props.file}" does not exists. Please provide valid file.`
        )
      );
    }
    ...
    loadFile(props.file)
      .then((data) => {
        const documentKey = Object.keys(data.files).find((key) =>
